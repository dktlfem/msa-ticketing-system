...
> payment-app의 핵심 설계 결정은 세 가지입니다.
>
> 첫째, 트랜잭션 경계입니다. `/confirm`에 @Transactional이 없습니다. TossPayments read-timeout이 10초인데, 이 동안 커넥션을 묶으면 HikariCP 풀이 고갈됩니다. PaymentWriter의 각 메서드가 독립 @Transactional을 가져 커넥션은 DB 작업 시점에만 획득됩니다.
>
> 둘째, 이중 결제 2중 방어입니다. Redis SETNX로 동시 요청을 1차 차단하고, DB UNIQUE KEY(reservation_id, order_id)가 2차 방어합니다. Redis 장애 시에도 DB UK가 최종 안전망입니다.
>
> 셋째, 보상 트랜잭션입니다. PG 승인 후 booking confirm이 실패하면 TossPayments cancel을 호출합니다. 이 취소도 실패하면 CANCEL_FAILED로 전이하고 [CRITICAL] 로그를 남깁니다. 현재는 수동 처리가 필요하며, Alertmanager 자동 알림이 다음 우선순위입니다.
