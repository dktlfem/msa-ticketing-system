...
>
> 관측성 스택 개요(Prometheus, Grafana, Jaeger, Alertmanager 설정)는 [`docs/05-observability.md`](../05-observability.md),
> payment confirm 흐름의 trace 절차는 [`docs/observability/observability.md`](../observability/observability.md),
...
|---------|------------|--------------------------|---------|
| MySQL single instance 장애 | booking, payment availability SLO 즉시 위반 | 빠름 (수분 내 전체 소진) | `hikaricp.connections.active` = 0, 5xx 급증 |
...
- `PaymentService.confirmPayment()` 성공 시: `Counter("payment.approved")` increment
- `PaymentService.confirmPayment()` PG 오류 시: `Counter("payment.pg_error", "code", failureCode)` increment
- CANCEL_FAILED 전이 시: `Counter("payment.cancel_failed")` increment

이 counter들이 추가되면 다음 Prometheus 쿼리로 Business SLI를 실시간 측정할 수 있다:

```promql
-- APPROVED 비율 (5분 window)
rate(payment_approved_total[5m])
/
(rate(payment_approved_total[5m]) + rate(payment_pg_error_total[5m]))

-- CANCEL_FAILED 건수 (1시간 window)
increase(payment_cancel_failed_total[1h])
```
...
별도의 SLO 대시보드가 필요하다. 현재 `docs/05-observability.md`의 Grafana panel들은 지표 모니터링 목적이지 SLO 준수 현황을 한눈에 볼 수 없다.
...
1. **Micrometer counter 등록**: payment-app에 APPROVED, PG 오류, CANCEL_FAILED counter를 추가해 Business SLI를 Prometheus에서 실시간 측정한다.
...
SLI는 실제로 측정하는 지표이고, SLO는 그 지표의 목표값이며, Error Budget은 "1 - SLO"로 계산한 허용 실패 범위입니다.

이 시스템에서 SLI를 네 카테고리로 정의했습니다. 가용성은 5xx가 아닌 응답 비율이고, 레이턴시는 엔드포인트별 P95/P99이며, 오류율은 5xx 비율과 PG 오류 비율을 분리해서 봅니다. 그리고 가장 중요한 Business SLI로 payment APPROVED 비율과 CANCEL_FAILED 건수를 정의했습니다.

SLO는 현재 모두 proposed 상태입니다. payment /confirm에 가장 엄격한 99.9% availability를 제안했는데, 이는 월 43분의 Error Budget을 의미합니다. CANCEL_FAILED는 0건/일 SLO로 단 1건이라도 발생하면 즉시 P0 대응이 필요합니다.

한계도 명확합니다. MySQL single instance와 Redis single node 구성에서 99.9%를 지키려면 재시작 한 번에 Error Budget이 크게 소진될 수 있습니다. Business SLI인 APPROVED 비율은 현재 DB 쿼리로만 측정 가능하고 Prometheus에서 실시간으로 볼 수 없어서, Micrometer counter 추가가 다음 우선순위입니다.
