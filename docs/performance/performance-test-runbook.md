...
> [`docs/performance/concurrency-test-runbook.md`](./concurrency-test-runbook.md)에 있으며 이 문서는 그 내용을 반복하지 않는다.
...
| 계층 | 도구 | 위치 | 검증 대상 | 참조 |
|------|------|------|----------|------|
| Layer 1: JVM 동시성 | CountDownLatch (JUnit) | 단일 JVM | 낙관적 락 충돌률, 멱등성 DB 정합성, 상태 전이 정확성 | [concurrency-test-runbook](./concurrency-test-runbook.md) |
...
현재 [`docs/performance/concurrency-test-runbook.md`](./concurrency-test-runbook.md)의 사전 준비 절차에서 언급한 것처럼 "waiting token 또는 reservation/payment fixture 준비"가 필요하다.
...
자세한 SQL 확인 쿼리는 [`docs/performance/concurrency-test-runbook.md` 섹션 7](./concurrency-test-runbook.md)을 참고한다.
...
2. **k6 시나리오 CI nightly job 통합**: [`docs/performance/concurrency-test-runbook.md`](./concurrency-test-runbook.md) 섹션 10에서 언급한 것처럼 CountDownLatch 테스트와 함께 k6 시나리오도 nightly로 실행해 P99 기준선을 지속적으로 추적한다.
