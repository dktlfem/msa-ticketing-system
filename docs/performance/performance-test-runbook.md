...
> 관측성 스택(Prometheus, Grafana, Jaeger) 개요는 [`docs/05-observability.md`](../05-observability.md),
> payment confirm 흐름 trace 절차는 [`docs/observability/observability.md`](../observability/observability.md)를 참고한다.
...
- Redis `processing` key의 TTL과 `setIfAbsent` 경쟁 구간이 핵심이다. 자세한 내용은 `docs/observability/observability.md`의 idempotency 흐름을 참고한다.
...
현재 `docs/07-performance-test-runbook.md`의 사전 준비 절차에서 언급한 것처럼 "waiting token 또는 reservation/payment fixture 준비"가 필요하다.
...
2. **k6 시나리오 CI nightly job 통합**: `docs/07-performance-test-runbook.md` 섹션 10에서 언급한 것처럼 CountDownLatch 테스트와 함께 k6 시나리오도 nightly로 실행해 P99 기준선을 지속적으로 추적한다.
...
이 시스템의 성능 테스트는 두 계층으로 구성됩니다.

첫 번째 계층은 CountDownLatch 기반 JVM 테스트로, 단일 JVM 내부에서 100개 스레드가 동시에 같은 좌석에 예약을 시도하거나, 동일 결제를 100번 confirm할 때 DB 정합성이 유지되는지 검증합니다.

두 번째 계층이 k6 HTTP 부하 테스트입니다. JVM 내부 테스트만으로는 알 수 없는 것들이 있습니다. SCG Netty 라우팅이 실제로 병목인지, TossPayments 10초 read timeout이 P99에 어떻게 반영되는지, HikariCP 10개 커넥션이 50 VU 아래에서 언제 고갈되는지를 실제 HTTP 요청으로만 측정할 수 있습니다.

k6 시나리오는 네 가지입니다. 좌석 선점 경쟁에서 P95 < 500ms와 5xx = 0을 검증하고, 결제 중복 confirm에서 PG 호출이 정확히 1회인지를 확인하고, E2E 흐름에서 단계별 레이턴시 분포를 측정하고, SCG throughput이 1000 req/s를 넘는지 검증합니다.

중요한 것은 k6 5xx = 0이 합격 기준이 아니라는 점입니다. 응답이 전부 409여도 DB에서 HOLD가 2건이라면 락이 뚫린 것입니다. 반드시 SQL로 최종 상태를 함께 확인합니다.
