...
> 전체 인프라 토폴로지와 CI/CD 파이프라인은 `docs/02-architecture-infrastructure.md` 참조.
> payment 상태 머신(READY → APPROVED/FAILED → REFUNDED/CANCEL_FAILED) 상세는 `docs/services/payment/payment-architecture.md` 참조.
...
MySQL (스테이징): 192.168.124.100:33066
...
| ticketing_concert | concert-app | concerts, seats |
| ticketing_booking | booking-app | bookings, reservations |
| ticketing_payment | payment-app | payments |
| ticketing_waitingroom | waitingroom-app | waiting_tokens |
...
mysql \
  ...
  -u root -p \
  ticketing_payment \
  < payments_backup_20240101_120000.sql
```
...
0 2 * * * root mysqldump -h 192.168.124.100 -P 33066 -u backup_user -p<password> \
