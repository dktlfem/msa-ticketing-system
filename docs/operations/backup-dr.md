...
**배경**: MSA 기반 티켓팅 시스템에서 결제 데이터는 고객 자산과 직결되므로 백업과 장애 복구 계획이 필요하다.

**현재 한계를 솔직하게**: 이 시스템은 staging 환경으로, `ddl-auto=update`로 스키마를 자동 관리하고, 정기 자동 백업이 없으며, Redis persistence 설정도 아직 확인되지 않았다. RPO/RTO도 명시적으로 정의되지 않았다.

**핵심 설계 선택**: 가장 위험한 케이스인 `CANCEL_FAILED` 상태에 집중했다. 이 상태는 결제 취소가 실패해 고객 돈이 묶인 상태로, 자동 감지 없이 수동 쿼리로만 확인 가능하다. 탐지되면 Toss Payments 콘솔에서 실제 취소 상태를 확인한 후 DB를 맞추는 순서로 처리한다.

**복구 우선순위**: ticketing_payment > ticketing_booking > 나머지 순서로 복구한다. Redis 장애는 대기열 재진입으로 자연 복구되므로 DB 복구보다 빠르게 서비스를 재개할 수 있다.

**개선 방향**: AWS RDS automated backup 활성화, cron 기반 mysqldump 자동화, Flyway 도입으로 스키마 이력 관리. 이 세 가지가 현재 가장 큰 백업/DR 갭이다.

---

## 추가 검토 필요 시나리오

> 아래 시나리오는 설계 문서(payment-architecture.md, seat-locking-design.md, queue-design.md, security-design.md)에서 언급된 장애 포인트 중 현재 backup-dr.md에 복구 절차가 없는 항목입니다. 각 항목은 DR 훈련 시나리오 혹은 Planned 작업으로 우선순위를 지정할 것을 권장합니다.

| 시나리오 | 영향 범위 | 심각도 | 현재 대응 여부 |
|---------|----------|--------|--------------|
| payment APPROVED + booking PENDING 불일치 상태 미복구 (reconciliation job 미구현) | 결제는 완료됐으나 예약 미확정 상태 다수 잔류 가능 — 고객 불이익 지속 | High | ❌ 미대응 |
| Redis RDB/AOF persistence 미설정 시 재시작 후 전체 데이터 소멸 | 대기열 순번 소멸 + idempotency 키 소멸 → 동시 재시도 시 PG 이중 호출 위험 | High | ❌ 미대응 (설정 미확인) |
| Jenkins CI/CD 서버 장애로 배포 파이프라인 전면 중단 | 긴급 패치 배포 불가 — 장애 대응 속도 저하 | Medium | ❌ 미대응 |
| Docker volume 마운트 누락으로 MySQL 컨테이너 재시작 시 데이터 소멸 | ticketing_* 모든 스키마 데이터 손실 | Critical | ❌ 미대응 |
| 부분 서비스 DB 손실 (특정 스키마만 장애) 후 서비스 간 정합성 복구 절차 부재 | 예: ticketing_booking 손실 시 payment APPROVED 건과 좌석 상태 불일치 | High | ❌ 미대응 |
| VPN 서버 장애로 원격 관리 접근 전체 불가 | Grafana/Jenkins/MySQL/Redis 접근 불가 — 장애 대응 수단 소멸 | Critical | ❌ 미대응 |
| AWS RDS 스냅샷 복구 후 서비스 엔드포인트 재설정 절차 미문서화 | 전체 복구 시 환경변수(SPRING_DATASOURCE_URL) 업데이트 누락 위험 | Medium | ❌ 미대응 |
| seat HOLD 상태 잔류 + booking-app 스케줄러 미복구 (concert-app 좌석 영구 잠금) | 특정 좌석이 HOLD에서 벗어나지 못해 판매 불가 | Medium | ❌ 미대응 |
