...
// 면접 활용: "Redis rate-limiter round-trip + JWT 검증을 포함한 전체 필터 체인이
...
export function handleSummary(data) {
    ...
    if (overallPass) {
        ...
        passNotes.push(
            `면접 포인트: "SCG가 병목이 아닌 근거로, SCG 경유 P95=${scgP95.toFixed(1)}ms vs ` +
...
<style>
  ...
  .highlight { font-size: 1.8rem; font-weight: 700; color: ${ohColor}; }
</style>
</head>
<body>
...
<div style="text-align:center; margin:24px 0; padding:20px; background:#f8fafc; border-radius:8px;">
  <div style="color:#6b7280; font-size:0.9rem;">SCG 경유 P95 − 직접 호출 P95</div>
  <div class="highlight">${overheadP95.toFixed(2)} ms</div>
  <div style="color:${ohColor}; font-size:0.85rem;">${passOverhead ? '✓ 목표 달성 (< 10ms)' : '✗ 목표 초과 (< 10ms)'}</div>
</div>
...
<p class="meta">Generated by k6 scenario4-filter-latency.js | rate=15req/s, duration=60s</p>
    ...
    const consoleMsg = [
        ...
        `  산출물: ${RESULT_DIR}/scenario4-filter-latency_${RUN_TAG}.*`,
        '',
    ].join('\n');

    return {
        stdout: consoleMsg,
        [`${RESULT_DIR}/scenario4-filter-latency_${RUN_TAG}.json`]: JSON.stringify(jsonReport, null, 2),
        [`${RESULT_DIR}/scenario4-filter-latency_${RUN_TAG}.csv`]:  csv,
        [`${RESULT_DIR}/scenario4-filter-latency_${RUN_TAG}.html`]: html,
    };
}
