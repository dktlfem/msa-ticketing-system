...
// ADR: DB에 실제 존재하는 reservation_id 사용. 존재하지 않는 ID는 payment-app에서 비즈니스 에러 발생.
// ADR: payment-app은 예약 소유자(userId)를 검증하므로, reservation과 userId를 반드시 매칭해야 함.
const SHARED_RESERVATION_ID  = 4;
const SHARED_RESERVATION_OWNER = 1;   // reservation_id=4의 소유자
// Phase 3용: DB에 존재하는 예약 ID + 소유자 매핑 (ticketing_booking.reservations 테이블 기준)
const RESERVATION_MAP = [
    { reservationId: 4,  userId: 1 },
    { reservationId: 6,  userId: 2 },
    { reservationId: 43, userId: 100 },
    { reservationId: 44, userId: 200 },
];
...
export const options = {
    scenarios: {
        ...
        // ADR: DB에 존재하는 예약이 4건이므로 VU 4로 제한. 각 VU가 고유 reservationId + 고유 Key 사용.
        unique_keys: {
            executor: 'per-vu-iterations',
            vus: 4,
            iterations: 1,
...
export function setup() {
    ...
    console.log(`  Phase 3: ${RESERVATION_MAP.length} VU × 고유 Key (reservations=${JSON.stringify(RESERVATION_MAP)})`);
...
export function concurrentDuplicatePhase() {
    const vuId = __VU;
    // ADR: 모든 VU가 동일한 예약 소유자 userId로 요청해야 R001(접근 권한 없음) 회피.
    // 멱등성 검증 핵심: 동일 Key + 동일 userId → 정확히 1건만 처리.
    const userId = SHARED_RESERVATION_OWNER;
...
export function sequentialRetryPhase() {
    const iter = __ITER;
    // ADR: Phase 1과 동일한 Key + 동일한 소유자로 재시도 → 캐시된 응답 반환 확인.
    const userId = SHARED_RESERVATION_OWNER;
    const token = generateJwt(userId);
    const passport = generateAuthPassport(userId);
...
export function uniqueKeysPhase() {
    const vuId = __VU;
    // ADR: 예약 소유자와 reservationId를 정확히 매칭. userId 불일치 시 R001 에러 발생.
    const vuIndex = (vuId - 1) % RESERVATION_MAP.length;
    const entry = RESERVATION_MAP[vuIndex];
