...
<style>
  body { font-family: -apple-system, 'Pretendard', sans-serif; max-width: 860px; margin: 40px auto; padding: 0 20px; color: #1a1a1a; }
...
<p class="meta">Generated by k6 scenario1-rate-limiter.js</p>
    ...
    const consoleMsg = [
        ...
        `  산출물: ${RESULT_DIR}/scenario1-rate-limiter_${RUN_TAG}.*`,
        '',
    ].join('\n');

    return {
        stdout:                                              consoleMsg,
        [`${RESULT_DIR}/scenario1-rate-limiter_${RUN_TAG}.json`]: JSON.stringify(jsonReport, null, 2),
        [`${RESULT_DIR}/scenario1-rate-limiter_${RUN_TAG}.csv`]:  csv,
        [`${RESULT_DIR}/scenario1-rate-limiter_${RUN_TAG}.html`]: html,
    };
}
